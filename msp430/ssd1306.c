...
void ssd1306_clearDisplay(void) {
    ...
    int syc;
    for (syc = 69; syc > 0; syc--) {                                          // count down for loops when possible for ULP
        uint8_t x;
        for(x = 16; x > 0; x--) {
                buffer[x-1] = 0x0;
        }

        i2c_multiwrite(OLED,0x40,buffer, 16);
    }
...
void ssd1306_printText(uint8_t x, uint8_t y, char *ptString) {
    ...
    while (*ptString != '\0') {
        ...
        uint8_t i;
        for(i = 0; i< 5; i++) {
            buffer[i] = font_5x7[*ptString - ' '][i];
        }

        buffer[5] = 0x0;

        i2c_multiwrite(OLED,0x40,buffer, 6);
        ptString++;
        x+=6;
    }
